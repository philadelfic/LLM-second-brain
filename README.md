...
- Суммаризация: отдельная внешняя LLM (адрес и модель — в env) строит короткое
  `summary` каждой заметки; в поиске выдача дополняется `snippet`
  (фрагмент лучшего чанка), в списках — только `summary`. Генерация —
  фоновая, не блокирует
...
Вся конфигурация — в `docker-compose.yml`, блок `environment` (все 34
...
- **Поиск**: KNN по чанкам (топ-50) → агрегация до заметок: лучший чанк
  задаёт `cosine` и `snippet` (первые `SNIPPET_CHARS` символов чанка);
...
- **Фаза 7 — чанк-индексация: шаги 1–7 выполнены (деплой — шаг 8).**
...
- **Далее:** деплой Фазы 7; после — Фаза 6 (опционально, не отменена) —
  Function-фильтр Open WebUI для авто-подмешивания топ-K суммари к
  первому сообщению чата.
