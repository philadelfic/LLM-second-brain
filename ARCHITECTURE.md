...
- **EmbeddingService** — клиент к Ollama векторизации (localhost):
  `/api/embed`, batch, таймауты, один ретрай в синхронном пути.
- **SummaryService** — клиент к Ollama суммаризации (localhost,
...
```
валидация длины
  → кодирование текста (Ollama localhost, sync, 1 ретрай)
...
- **Интеграционные** (маркер `@pytest.mark.integration`): живая Ollama
  векторизации (localhost, `qwen3-embedding:8b`) и живая Ollama
  суммаризации (localhost, `ornith-1.5:35b`): качество поиска на
