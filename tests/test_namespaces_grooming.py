...
default-заметок (hint_path) и вердиктов promotions — ничего не теряется
(US-11). Работают с листьями глубины 1..3 (lsb-0005-04).
"""
...
def _row(settings, note_id: int) -> dict:
    with session(settings) as conn:
        row = conn.execute(
            "SELECT namespace, hint_path, vector_status "
            "FROM notes WHERE id = ?",
            (note_id,),
        ).fetchone()
    return dict(row)
...
def _verdict(settings, path: str):
    with session(settings) as conn:
        return conn.execute(
...
class TestRename:
    def test_rename_root_relocates_everything(self, settings) -> None:
        """Rename корня: узел + дети + заметки + hint_path + promotions."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/subo", "СУБО 2020: сервисы HR.")
        namespaces.create("work/subo/deep", "Глубина 3.")
        note_leaf = _seed_note(settings, "work/subo")
        note_root = _seed_note(settings, "work")
        note_hint = _seed_note(settings, "default", hint_path="work")
        note_deep = _seed_note(settings, "default", hint_path="work/subo/deep")
        with session(settings) as conn, transaction(conn):
            conn.execute(
        ...
        with session(settings) as conn:
            hint = conn.execute(
                "SELECT hint_path FROM notes WHERE id = ?", (note_hint,)
            ).fetchone()
            deep = conn.execute(
                "SELECT hint_path FROM notes WHERE id = ?", (note_deep,)
            ).fetchone()
            verdict = _verdict(settings, "biz/subo")
        assert hint["hint_path"] == "biz"
        assert deep["hint_path"] == "biz/subo/deep"  # потомки глубины 3 тоже
        assert verdict is not None  # domain и canonical переехали, не потерялись

    def test_rename_leaf_relocates_hint_path(self, settings) -> None:
        """Rename листа (глубина 2): namespace заметок + hint_path default-заметок."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/subo", "СУБО 2020: сервисы HR.")
        note_leaf = _seed_note(settings, "work/subo")
        note_hint = _seed_note(settings, "default", hint_path="work/subo")
        ...
        assert hint["hint_path"] == "work/sbos2020"

    def test_rename_depth3_leaf_canonicalizes_hint_path(self, settings) -> None:
        """Rename листа глубины 3: hint_path default-заметок канонизируется."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/proj", "Проект.")
        namespaces.create("work/proj/task", "Задача.")
        note_leaf = _seed_note(settings, "work/proj/task")
        note_hint = _seed_note(settings, "default", hint_path="work/proj/task")
        with session(settings) as conn, transaction(conn):
            conn.execute(
                "INSERT INTO promotions (hint_path, status, canonical_path) "
                "VALUES ('work/proj/task', 'merged', 'work/proj/task')"
            )
        namespaces.rename("work/proj/task", "work/proj/renamed")
        leaf = _row(settings, note_leaf)
        with session(settings) as conn:
            hint = conn.execute(
                "SELECT hint_path FROM notes WHERE id = ?", (note_hint,)
            ).fetchone()
            verdict = _verdict(settings, "work/proj/renamed")
        assert leaf["namespace"] == "work/proj/renamed"
        assert leaf["vector_status"] == "pending"
        assert hint["hint_path"] == "work/proj/renamed"  # канонизирован
        assert verdict is not None  # promotions следуют за перекладкой
...
class TestMergeNode:
    def test_merge_leaf_into_leaf_canonicalizes_hint(self, settings) -> None:
        """Слияние листа (глубина 2): hint_path канонизируется к полному пути цели."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/subo", "СУБО 2020: сервисы HR.")
        ...
        with session(settings) as conn:
            row = _row(settings, note_id)
            verdict = _verdict(settings, "work/other")
            gone = _verdict(settings, "work/subo")
        assert row["namespace"] == "work/other"
        assert row["hint_path"] == "work/other"  # hint канонизирован к цели
        assert row["vector_status"] == "pending"
        # Вердикт источника мигрировал на ключ цели (canonical_path тоже).
    ...
    def test_merge_leaf_into_root_nulls_hint(self, settings) -> None:
        ...
        assert row["hint_path"] is None  # «общая для домена»

    def test_merge_depth3_canonicalizes_hint_path(self, settings) -> None:
        """Слияние листа глубины 3: hint_path канонизируется к полному пути цели."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/proj", "Проект.")
        namespaces.create("work/proj/task", "Задача.")
        namespaces.create("work/proj/other", "Другой лист.")
        note_id = _seed_note(settings, "work/proj/task")
        with session(settings) as conn, transaction(conn):
            conn.execute(
                "INSERT INTO promotions (hint_path, status, canonical_path) "
                "VALUES ('work/proj/task', 'rejected', NULL)"
            )
        result = namespaces.merge_node("work/proj/task", "work/proj/other")
        assert result["moved"] == 1
        assert namespaces.get("work/proj/task") is None
        row = _row(settings, note_id)
        with session(settings) as conn:
            verdict = _verdict(settings, "work/proj/other")
            gone = _verdict(settings, "work/proj/task")
        assert row["namespace"] == "work/proj/other"
        assert row["hint_path"] == "work/proj/other"
        assert row["vector_status"] == "pending"
        assert verdict is not None and verdict["canonical_path"] is None
        assert gone is None  # ключ источника мигрировал в цель

    def test_no_note_loss_on_depth3_merge(self, settings) -> None:
        """Слияние глубины 3 не теряет заметок (вектор-status pending сохранён)."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/proj", "Проект.")
        namespaces.create("work/proj/task", "Задача.")
        namespaces.create("work/proj/other", "Другой лист.")
        ids = [_seed_note(settings, "work/proj/task") for _ in range(3)]
        namespaces.merge_node("work/proj/task", "work/proj/other")
        with session(settings) as conn:
            rows = conn.execute(
                "SELECT namespace, hint_path, vector_status FROM notes "
                "WHERE id IN (?, ?, ?)",
                (*ids,),
            ).fetchall()
        assert len(rows) == 3  # ничего не потеряно
        assert all(r["namespace"] == "work/proj/other" for r in rows)
        assert all(r["hint_path"] == "work/proj/other" for r in rows)
...
class TestDeleteNode:
    def test_delete_leaf_moves_notes_to_root(self, settings) -> None:
        """Удаление листа (глубина 2): заметки/вердикты → родительский корень."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/subo", "СУБО 2020.")
        note_id = _seed_note(settings, "work/subo")
        ...
        row = _row(settings, note_id)
        with session(settings) as conn:
            verdict = _verdict(settings, "work")
            gone = _verdict(settings, "work/subo")
        assert row["namespace"] == "work"
        assert row["hint_path"] is None  # общая для домена
        assert row["vector_status"] == "pending"
        assert verdict is not None and verdict["canonical_path"] == "work"
        assert gone is None  # вердикт листа мигрировал в родителя

    def test_delete_depth3_leaf_moves_to_root(self, settings) -> None:
        """Удаление листа глубины 3: заметки/вердикты → домен-корень, hint NULL."""
        namespaces = NamespaceService(settings)
        namespaces.create("work", "Рабочие заметки.")
        namespaces.create("work/proj", "Проект.")
        namespaces.create("work/proj/task", "Задача.")
        ...
        result = namespaces.delete_node("work/proj/task")
        assert result["moved"] == 1
        assert namespaces.get("work/proj/task") is None
        row = _row(settings, note_id)
        with session(settings) as conn:
            verdict = _verdict(settings, "work")
            gone = _verdict(settings, "work/proj/task")
        assert row["namespace"] == "work"  # домен-корень
        assert row["hint_path"] is None  # общая для домена
        assert row["vector_status"] == "pending"
        assert verdict is not None and verdict["canonical_path"] == "work"
        assert gone is None
